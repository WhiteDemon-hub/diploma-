<template>
    <div class="m-5" v-if="task">
        ...
        <div class="d-flex flex-column w-75 m-auto" v-else>
            ...
            <button v-if="!task.end" @click="Next" type="button" class="btn btn-primary mr-2 mt-2 mb-2" style="width: 150px">Далее</button>
            <button v-if="task.end" @click="End" type="button" class="btn btn-primary mr-2 mt-2 mb-2" style="width: 150px">На главную</button>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
      ...
      async Check()
      {
          ...
          await axios 
          .post('/test/js/basic/' + this.$route.params.id, data)
          .then(async response => {
      ...
      async Next()
      {
        ...
        this.$router.push({ path: `/js/basic/${Number(this.$route.params.id) + 1}` })
        await this.LoadPage();
        this.$forceUpdate();
      },
      async LoadPage()
