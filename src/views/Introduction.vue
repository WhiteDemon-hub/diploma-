<template>
    <div class="m-5" v-if="task">
        ...
        <div v-if="task.type == 'task'">
            ...
            <div class="d-flex row m-5 flex-wrap">
...
export default {
    ...
    methods: {
      ...
      async Check()
      {
          ...
          await axios 
          .post('/test/js/introduction/' + this.$route.params.id, data)
          .then(async response => {
      ...
      async Next()
      {
        ...
        this.$router.push({ path: `/js/introduction/${Number(this.$route.params.id) + 1}` })
        await this.LoadPage();
        this.$forceUpdate();
      },
      async LoadPage()
